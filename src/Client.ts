...
export default class Client {
    ...
    async send<T = any>(path: string, reqOptions: SendOptions): Promise<T> {
        ...
        if (this.isCustomFormData(options.body)) {
            const data = options.body.getData()
            options.body = data.buffer
            options.headers = Object.assign({}, options.headers, {
                'Content-Type':  data.contentType
            });
        } else if (!this.isFormData(options.body)) {
    ...
    private isCustomFormData(body: any): boolean {
        return body && body instanceof CustomFormData
    }
